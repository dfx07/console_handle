...
typedef struct _tagMonitorInfo
{
	...
	DWORD dwDisfreq;
	DWORD dwVersion;
	WCHAR strName[MAX_DEVICENAME];
} tagMonitorInfo;

/***********************************************************************************
*! @brief  : Read bytes data file
*! @author : thuong.nv - [Date] : 2022.10.03
...
*! @author : thuong.nv - [Date] : 2022.10.08
*! @param  : [In] void
*! @return : struct MonitoInfo
*! @note   : N/A
***********************************************************************************/
tagMonitorInfo get_monitorinfo()
{
	tagMonitorInfo infor;
	ZeroMemory(&infor, sizeof(tagMonitorInfo));
	DEVMODE devmode;
	...
	if (bResult)
	{
		infor.dwWidth   = devmode.dmPelsWidth;
		infor.dwHeight  = devmode.dmPelsHeight;
		infor.dwDisfreq = devmode.dmDisplayFrequency;
		infor.dwVersion = devmode.dmDriverVersion;
		memcpy_s(infor.strName, MAX_DEVICENAME, devmode.dmDeviceName, MAX_DEVICENAME);
	}
	return infor;
}
...
*! @author : thuong.nv - [Date] : 2022.10.07
*! @param  : fpath : path
...
*! @param  : void
*! @return : path exec
*! @author : thuong.nv  - [Date] : 2023.05.25
*! @note   : N/A
***********************************************************************************/
